...
Sanctions screening：
俄乌战争之后 EU 制裁名单爆炸式增长,银行的处理量翻倍，有无数的sanctions list需要被check:
...
**北欧大银行每天处理的 sanctions hit 在 1 万到 10 万之间,其中 false positive rate (误报率) 大概在 97–99%**。也就是说,几乎所有 hit 都是无关的同名同姓、拼写相近、地址类似。但每一个 hit 都必须被人 review,因为漏掉一个真的命中,后果是天文数字的罚款 + 监管风险。
...
GenAI 的价值是什么:**用 LLM 处理 unstructured data (非结构化数据,比如 KYC 文档、新闻、过往 case notes),帮 analyst 把每个 hit 的初步调查压缩到 5-10 分钟**。这不是替代 analyst,是给 analyst 一个 co-pilot,让他们专注在真正需要人判断的复杂 case 上。

---

## 2. Architecture: Three boundaries to settle first

Before drawing the agent diagram, three boundaries need to be clear in my head.

### Boundary 1: Don't touch detection

The existing sanctions screening engine — based on fuzzy name match, phonetic match, and threshold rules — should stay as is. This layer is deterministic, auditable, and explainable. Regulators expect every alert to be traceable to a specific rule that fired. Putting an LLM here adds hallucination risk for no real gain.

GenAI only enters the picture after an alert is raised.

### Boundary 2: Agents don't make the final call

Auto-clearing a sanctions hit carries legal weight. If an agent clears a real match, the bank is on the hook for regulatory penalties and reputational damage. So the agent's role is to produce a recommendation, the supporting evidence, and a confidence score. A human analyst is the one who actually presses the button.

This also aligns with how the EU AI Act treats high-risk AI systems — meaningful human oversight isn't a rubber stamp; the human has to actually look at the evidence and decide.

### Boundary 3: Every step has to be traceable

A regulatory audit six months later might ask: why was that alert cleared? Which means every agent step needs to capture:

- Timestamp
- Which tools were called
- Which source documents were used
- The reasoning the agent produced

This is the standard audit trail requirement, and it has to be baked into the architecture from the start — it can't be added on later.
...
## 8. 在 Nordic / EU regulatory context 下的几个考虑
...
- The detection layer stays untouched
- Agents recommend, humans decide — always
- Every step is traceable; the audit trail is a first-class 
  state field, not an afterthought
- Evaluation must include citation grounding — LLM 
  hallucination is fatal in this domain
- Data residency and EU AI Act compliance are day-one design 
  requirements, not retrofits

This is what a multi-agent system can credibly look like in a 
